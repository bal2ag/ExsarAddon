...
- **Icons.** Static path (`icon`), item-derived (`iconItem` via `GetItemInfo`, `GET_ITEM_INFO_RECEIVED` retry), spell-derived (`iconSpell` via `GetSpellTexture`, `SPELLS_CHANGED` retry), **pet-state-dynamic** (`dynamicIcon`: map of `PetStateKey` → icon spec, re-resolved each poll — e.g. the all-in-one Mend/Revive/Call button; the engine's `petState` treats `not UnitExists("pet") and IsUsableSpell("Revive Pet")` as a **dead** pet, so a token-absent revivable pet shows the Revive icon rather than the Call whistle and the `requires` gating stays correct), equipped-trinket (`trinketSlot`), or first-known-spell (`spells`). `#showtooltip` auto-icon magic only applies to Blizzard action-bar buttons, not our secure buttons, hence the manual resolve.
...
**`PetManagementWidget.lua`** = `PET_ACTIONS` (macro-kind, horizontal layout) + a `CreateActionBar` call. Pet actions: all-in-one Mend/Revive/Call (`dynamicIcon`, never greyed; its macro casts `Revive Pet` **unconditionally** before the `[nopet] Call Pet` fallback so a dead pet is revived even when the "pet" unit token momentarily reads as absent around a death in combat — the Revive's GCD blocks the trailing Call so there's no double-summon), Send to Attack, Recall, Dismiss, Steam Tonk save trick (`cooldownItem` + `countCharges`, `requires="notdead"`), Feed Pet (`countItem = "Clefthoof Ribs"`), and Devilsaur Tooth Prep (see below). All macros are **fully addon-controlled** — no user override, no config-panel macro editing, no persisted macro DB. 7 slots, `EXSAR_PET_ACTION1..7`, "ExsarAddon Pet" category. `MAX_SLOTS` must stay equal to the `EXSAR_PET_ACTION` binding count in `Bindings.xml` — grow `PET_ACTIONS`, `MAX_SLOTS`, and `Bindings.xml` together.
...
- `ExsarAddonDB.petManagement` — position (x, y), scale, locked, devilsaurSwap (`{ bag, slot }` of the Devilsaur Tooth's last-known bag location; captured at stage-1 of the prep workflow so click 2 can swap the previous trinket back to the same coords)
