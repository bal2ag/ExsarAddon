...
| `PetManagementWidget.lua` | Keybindable mini action bar of pet-management actions. Each slot is a secure `type="macro"` button driven by macrotext; a slot can carry a conditional smart-macro (e.g. Call/Revive/Dismiss in one button). Macro bodies are **fully addon-controlled** — they live in the `PET_ACTIONS` table and there is no user override (no config-panel macro editing, no persisted macro DB). One slot — Devilsaur Tooth Prep — is a two-click boss-prep workflow (equip → use+swap) with yellow/red warning glow, see structure note. Each slot is keybindable via Blizzard's Key Bindings UI (see `Bindings.xml`, `EXSAR_PET_ACTION1..7`, "ExsarAddon Pet" category); the bound key shows top-right of the icon |
...
**`TrapsWidget.lua`** = `TRAPS` (macro-kind, horizontal layout) + a `CreateActionBar` call. 5 Hunter traps (Frost, Freezing, Immolation, Explosive, Snake), each `/cast <Trap>` (no rank → highest known) with `cooldownSpell` so each slot shows its cooldown sweep + countdown. `EXSAR_TRAP1..5`, "ExsarAddon Traps" category.

**`UtilitiesWidget.lua`** = `ACTIONS` (mixed, horizontal layout, `dimOnCooldown = true`) + a `CreateActionBar` call. 9 slots: Misdirection (MM talent → hides), Feign Death, Tranquilizing Shot, Eyes of the Beast, Shadowmeld (NElf racial → hides), Flare (all via `ability()`), a fishing-setup macro, and 2H / dual-1H weapon-swap macros (hardcoded `iconItem` weapons). The fishing + weapon slots use `isActive` (via an `equipped(slot, name)` check on `GetInventoryItemLink`) + `border = true` so the gold ring lights the slot whose gear is currently equipped — the dual-1H slot only when BOTH hands match; refreshed on `PLAYER_EQUIPMENT_CHANGED`. `EXSAR_UTILITY1..9`, "ExsarAddon Utility" category.

**`CoreCombatWidget.lua`** = `ACTIONS` (mixed, horizontal layout, `dimOnCooldown = true`) + a `CreateActionBar` call. 15 slots: Auto Shot start-attack macro, Raptor Strike weave macro, then 13 abilities via the `ability(name)` helper (`spells = { name }` → cast/icon/tooltip/cooldown auto, hides if unknown). Only Aimed Shot is talent-gated (hides off-MM); the rest are baseline. `EXSAR_COMBAT1..15`, "ExsarAddon Combat" category.

**`CooldownsWidget.lua`** = `COOLDOWNS` (mixed, horizontal layout, `dimOnCooldown = true`) + a `CreateActionBar` call. 7 slots: top trinket (`trinketSlot = 13`), bottom trinket (`trinketSlot = 14`), Bestial Wrath ∪ Readiness (`spells` — by spec), Rapid Fire (`spells`, always known), Intimidation (`spells` — hidden unless BM), then two burst macros — Bestial Wrath + bottom trinket and Rapid Fire + top trinket (`macro` + `iconSpell` + `cooldownSpell`/`cooldownTrinket`). Standalone slots kept alongside the macros so each can be bound separately. `EXSAR_COOLDOWN1..7`, "ExsarAddon Cooldowns" category.
