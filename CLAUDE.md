...
- **Icons.** Static path (`icon`), item-derived (`iconItem` via `GetItemInfo`, `GET_ITEM_INFO_RECEIVED` retry), spell-derived (`iconSpell` via `GetSpellTexture`, `SPELLS_CHANGED` retry), **pet-state-dynamic** (`dynamicIcon`: map of `PetStateKey` → icon spec, re-resolved each poll — e.g. the all-in-one Mend/Revive/Call button), equipped-trinket (`trinketSlot`), or first-known-spell (`spells`). `#showtooltip` auto-icon magic only applies to Blizzard action-bar buttons, not our secure buttons, hence the manual resolve.
