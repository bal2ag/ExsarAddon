...
- **Cooldown sweep.** Item kind: the active item's own CD (read from bags via `C_Container.GetContainerItemCooldown` — TBC has no `GetItemCooldown`) or a player debuff (`cooldownDebuff`, e.g. Tinnitus for Drums, Recently Bandaged for bandages). Macro/spell kind: the longest-remaining of `cooldownItem` (resolved once via `GetItemInfoInstant`, classified by `CooldownState`), `cooldownSpell` (`GetSpellCooldown(spellName)` — e.g. a trap's cooldown), `cooldownDebuff`, and the **GCD** (`gcd = true`; probed with `GetSpellCooldown("Wing Clip")`, which has no CD of its own — same as `GlobalCooldownTracker`). Only a real item cooldown shows a countdown number; the brief GCD is sweep-only.
- **Icons.** Static path (`icon`), item-derived (`iconItem` via `GetItemInfo`, `GET_ITEM_INFO_RECEIVED` retry), spell-derived (`iconSpell` via `GetSpellTexture`, `SPELLS_CHANGED` retry), or **pet-state-dynamic** (`dynamicIcon`: map of `PetStateKey` → icon spec, re-resolved each poll — e.g. the all-in-one Mend/Revive/Call button). `#showtooltip` auto-icon magic only applies to Blizzard action-bar buttons, not our secure buttons, hence the manual resolve.
