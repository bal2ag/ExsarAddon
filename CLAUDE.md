...
| `RaidDebuffTracker.lua` | "Missing raid debuffs" caller — inverse of `TargetDebuffTracker`. Scans ALL debuffs on the target (raid debuffs come from other players, so no `"PLAYER"` filter; matched by name to catch every rank/source) and shows a Red-X-marked icon for each configured critical hunter-damage debuff that is ABSENT, so the user can call it out on Discord. Visible only in combat, in a raid, with a target, and only when something is actually missing. Each tracked debuff is independently toggleable in config (disable the ones the raid can't provide); whole widget also toggleable. Settings under `ExsarAddonDB.raidDebuffs` |
...
| `PetManagementWidget.lua` | Keybindable mini action bar of pet-management actions. Each slot is a secure `type="macro"` button driven by macrotext; a slot can carry a conditional smart-macro (e.g. Call/Revive/Dismiss in one button). Macro bodies use an **inline default + editable override** model: built-in defaults live in the `PET_ACTIONS` table, the user can override any slot's macro in the config panel, and overrides persist to `ExsarAddonDB.petManagement.macros[<key>]`. Each slot is keybindable via Blizzard's Key Bindings UI (see `Bindings.xml`, `EXSAR_PET_ACTION1..5`, "ExsarAddon Pet" category); the bound key shows top-right of the icon |
...
- **Caster matching (talent-gated debuffs):** some debuffs only deliver their hunter-relevant bonus when the caster is **talented**, and that talent is unreadable from the aura (the aura name/spellId is identical talented or not). Two such debuffs: **Improved Faerie Fire** (druid talent → +hit) and **Judgement of the Crusader** (paladin talent *Improved Seal of the Crusader* → +crit to all attacks against the judged target). The fix: identify the specced player(s) and match the debuff's **caster**. Driven by the **`CASTER_MATCHED`** table, keyed by debuff `key`: `{ talent, class, guidKey, nameKey, label }`. Adding another talent-gated debuff is one row.
  ...
  - **Auto-detect (talent inspect):** `RefreshInspectQueue` (on `PLAYER_ENTERING_WORLD`/`GROUP_ROSTER_UPDATE`) collects raid members of the `INSPECT_CLASSES` (union of `CASTER_MATCHED` classes) into `inspectWanted`; a 2s poller (`ProcessInspectQueue`, combat- and throttle-aware, one inspect in flight, 3s stale-pending timeout) calls `NotifyInspect` on a reachable member (`CanInspect`); `INSPECT_READY` → `OnInspectReady` reads inspect talents (`GetTalentInfo(t, i, true)`) into a `{name, rank}` list and, for every `CASTER_MATCHED` key whose `class` matches the inspected unit, uses `ExsarLogic.FindTalentRank` (pure, unit-tested) to check that key's talent rank > 0, caching the GUID in the in-memory `autoGuids[key]` / `autoNames` (not persisted — re-detected each session to handle respecs). Only needs to catch a player in inspect range (~28yd) **once**; the cached GUID then matches their debuff anywhere.
...
- **`unitCaster` reliability for raid members is unverified in-game (Anniversary 2.5.5); likewise whether `GetTalentInfo(..., true)` exposes inspect talents.** If `unitCaster` proves frequently `nil`, the robust fallback is combat-log `SPELL_AURA_APPLIED` source tracking; if inspect talents are unavailable, auto-detect degrades to manual-only.
- Each slot shows the debuff icon plus the standard **Red X** effect (inactive/missing) and a 1px red border **Active Glow**; slots only `Show()` when their debuff is missing. Horizontal left-to-right row.
...
- Config: enable-widget checkbox, scale, lock, one checkbox per tracked debuff (default ON, disable per raid composition), reset button. Per-debuff state persists to `ExsarAddonDB.raidDebuffs.debuffs[<key>]`.
...
- **Macro source = inline default + editable override.** `PET_ACTIONS` is the action table (`{ key, name, icon|iconItem, macro, requires }`); `key` is a stable id used for the override map so reordering the table never scrambles saved overrides. `ExsarLogic.ResolveActionMacro(default, override)` (pure, unit-tested) returns the override when it has non-whitespace content, else the default. Overrides persist to `ExsarAddonDB.petManagement.macros[<key>]`. The config panel renders one `ExsarUI.AddMacroEditBox` per action; clearing the box (or restoring the exact default text) drops the override. `ApplyMacros()` pushes the resolved macrotext onto each button — `SetAttribute` is protected, so it is skipped under `InCombatLockdown()` and re-applied on `PLAYER_REGEN_ENABLED`.
...
- `ExsarAddonDB.petManagement` — position (x, y), scale, locked, macros (per-action macrotext override map, keyed by action `key`)
- `ExsarAddonDB.raidDebuffs` — position (x, y), scale, locked, enabled, debuffs (per-debuff enable map keyed by debuff `key`; default ON, stores `false` only when disabled), ffDruidGuid, ffDruidName (designated Improved Faerie Fire druid), jocPaladinGuid, jocPaladinName (designated Improved Seal of the Crusader paladin) — both captured from target; the persisted guid/name keys come from the `CASTER_MATCHED` table
