...
- **Mechanism vs policy.** The engine owns *mechanism* — secure `SecureActionButtonTemplate` slots (`RegisterForClicks("AnyUp","AnyDown")`), layout, sweep/count/hotkey-label rendering, **Dimmed** grey-out, the keybind bridge, slash reset, scale/lock/reset config rows, `RegisterModule`, event+poller wiring. *Policy* is the per-slot action spec: declarative fields cover the common cases (`icon`/`iconItem`/`iconSpell`/`dynamicIcon`, `spells` [talent-gated, see below], `trinketSlot` [equipped 13/14], `ranks`, `alternates`, `countItem`/`countCharges`, `cooldownItem`/`cooldownSpell`/`cooldownTrinket`/`cooldownDebuff`/`gcd`, `requires`, `activeBuff`, `hideWhenEmpty`/`groupFallback`), the bar-level `dimOnCooldown` (dim macro/spell slots while a real CD runs), and **escape-hook callbacks** carry the genuinely bespoke 20% (`resolveActive`, `getCooldown`, `getCount`, `resolveIcon`, `isEnabled`, `isActive`, `tooltip`). `activeBuff` (a player buff name) / `isActive(s)` compute a per-slot `s.active`, which drives the chosen active indicator (`opts.border` gold ring or `opts.activeAnts` marching-ants border — animated on a dedicated always-on ticker). Action/spell slots highlight **only** when they define one of these predicates (e.g. the current aspect via `activeBuff`; the equipped weapon/pole via `isActive` in `UtilitiesWidget`); slots with neither never highlight, so enabling `border`/`activeAnts` on a bar lights up only the slots that opt in. When tempted to add an engine flag, prefer a per-slot closure unless a *second* bar would want it. Pure selection logic stays in `ExsarLogic` (`SelectBestRank`, `SelectFirstInStock`, `CooldownState`, `PetActionEnabled`, `PetStateKey`).
...
**`PetManagementWidget.lua`** = `PET_ACTIONS` (macro-kind, horizontal layout) + a `CreateActionBar` call. Pet actions: all-in-one Mend/Revive/Call (`dynamicIcon`, never greyed), Send to Attack, Recall, Dismiss, Steam Tonk save trick (`cooldownItem` + `countCharges`, `requires="notdead"`), Feed Pet (`countItem = "Clefthoof Ribs"`). All macros are **fully addon-controlled** — no user override, no config-panel macro editing, no persisted macro DB. 6 slots, `EXSAR_PET_ACTION1..6`, "ExsarAddon Pet" category. `MAX_SLOTS` must stay equal to the `EXSAR_PET_ACTION` binding count in `Bindings.xml` — grow `PET_ACTIONS`, `MAX_SLOTS`, and `Bindings.xml` together.
