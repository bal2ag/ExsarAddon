...
- **Tooltips.** Item slots show the item tooltip (`SetItemByID`); `trinketSlot` shows the equipped item (`SetInventoryItem`). Macro/spell slots show the **associated spell's** tooltip (`GetSpellLink` → `SetHyperlink`), resolved as resolved `spells` entry → `spell` → `iconSpell`; slots with no single spell (complex conditional macros — the all-in-one pet button, Send-to-Attack) fall back to name + macrotext. `tooltipSpell` overrides: a spell name forces that spell, or `false` forces the name + macrotext tooltip (used by the Cooldowns combo macros, which set `iconSpell` for the icon but want their full macro shown). A `tooltip(s, GameTooltip)` hook overrides everything.
