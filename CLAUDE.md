...
- **Cooldown sweep.** Item kind: the active item's own CD (read from bags via `C_Container.GetContainerItemCooldown` — TBC has no `GetItemCooldown`) or a player debuff (`cooldownDebuff`, e.g. Tinnitus for Drums, Recently Bandaged for bandages). Macro/spell kind: the longest-remaining of `cooldownItem` (resolved once via `GetItemInfoInstant`, classified by `CooldownState`), `cooldownDebuff`, and the **GCD** (`gcd = true`; probed with `GetSpellCooldown("Wing Clip")`, which has no CD of its own — same as `GlobalCooldownTracker`). Only a real item cooldown shows a countdown number; the brief GCD is sweep-only.
...
**`AspectsWidget.lua`** = `ASPECTS` (macro-kind, **vertical** layout, `activeAnts = true`) + a `CreateActionBar` call. 6 Hunter aspects (Hawk, Viper, Cheetah, Pack, Wild, Monkey), each `/cast Aspect of …` with `activeBuff` so the live aspect's marching-ants border animates. `EXSAR_ASPECT1..6`, "ExsarAddon Aspects" category. Same `MAX_SLOTS` ↔ `Bindings.xml` rule.
