...
- **Alternates:** Some items automatically swap to a better option based on context. The mana rune slot prefers Demonic Rune and falls back to Dark Rune when you're out of Demonic Runes. For mana and healing potions: inside Coilfang Reservoir instances (Serpentshrine Cavern, The Slave Pens, The Underbog, The Steamvault) the zone-specific Cenarion Mana/Healing Salve are preferred, and inside Tempest Keep instances the Bottled Nethergon Energy/Vapor are preferred; everywhere else Crystal Mana/Healing Potions are the top preference, then Auchenai potions, then the Super potion. The swap happens out of combat when you change zones or your bags update.
...
- **Clip feedback:** The overdue counter vanishes the instant the shot fires — exactly when you're looking at it. So each auto shot also gets an animated comic-pop scorecard (the same effect as the Melee Weave Helper's hit/miss text) that hangs around for a moment afterwards: a green **NO CLIP!** when the shot fired on time, or the delay in orange (e.g. **CLIP +0.34**), turning red past half a second. Glance at it between shots — if the number is popping often, you're clipping too much. The verdict is measured after the fact, comparing when the shot fired against when it was due, so it doesn't carry the small latency bias the live counter has to suppress. Toggle it with the "Clip feedback text" checkbox.
...
- **Hit / late / miss feedback:** An animated comic-pop **WEAVE HIT!** (green) flashes when a melee attack lands cleanly — instant confirmation you actually connected, which is hard to read off the swing timer alone. A **WEAVE MISS!** (red) pops when a weave window runs out without a swing; it waits for your auto shot to actually fire before calling it, so a swing that lands late is never mislabelled a miss. And **LATE - TAP STRAFE** (orange) pops when your swing did land, but only after a half-second delay.
...
- **Windfury proc flourish:** When a Windfury proc lands on your melee (from a shaman's Windfury Totem), an arcing crescent blade-slash sweeps up across the widget — curving left then thrusting back up the center and forward toward the enemy, anime style — with a "whoosh" — a bit of satisfying feedback that your weave caught the extra attacks. Purely cosmetic; toggle it with the "Windfury proc flourish (slash + whoosh)" checkbox. While the widget is unlocked, the slash loops silently so you can see exactly where it appears and position the widget accordingly.
...
- **Dark rim slider:** Adds a dark contour under the bright stroke so it stays readable against busy combat backdrops. Kept light by default. The bright layers are additive and can only add light, which washes out against spell FX; the rim supplies contrast the game world never draws. Set to 0 for the original pure-glow look.
...
- **Rocket Boots equip-and-swap:** A dedicated "Rocket Boots" slot lets you get the Goblin Rocket Boots speed boost without wearing them into a fight. First press equips the boots (remembering where your real boots go); the next press fires the boost. Once the boots go on cooldown, pressing again swaps your original boots back on. The swap-back is held until a cooldown is running on purpose — taking the boots off while the boost is active cancels it early, so firing the boost never rips them off in the same press. While the rocket boots are on your feet the slot shows an amber "swap back" warning. (Because you can't change gear in combat, boosting mid-fight leaves the rockets on until you're out of combat — the amber warning reminds you to swap.)
