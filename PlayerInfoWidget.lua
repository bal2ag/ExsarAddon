...
-- Each line's direction is from frame center through its border point and beyond.
-- Defs: { edge, position, length }
--   edge: "T"op, "B"ottom, "L"eft, "R"ight
--   position: 0..1 fraction along that edge (0=left/bottom, 1=right/top)
--   length: how far past the border the line extends (pixels)
local burstLines = {}
local BURST_DEFS = {
    -- Top edge
    { "T", 0.10, 22 },
...
local function UpdateBurstPositions()
    ...
    for _, b in ipairs(burstLines) do
        -- Border point relative to frame center
        local bx, by
        if b.edge == "T" then
            bx = -hw + b.pos * fw
            by = hh
        ...
        -- Direction: from center (0,0) through border point, normalized
        local mag = math.sqrt(bx * bx + by * by)
        if mag < 0.01 then mag = 1 end
        local dx, dy = bx / mag, by / mag
        -- End point: border point + direction * length
        local ex = bx + dx * b.len
        local ey = by + dy * b.len
        -- Line from border to end (radiating outward)
...
local function ShowLowHpWarning()
    ...
    -- Always reposition — frame height changes as auras are added/removed
